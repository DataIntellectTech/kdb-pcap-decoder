...
gettablerow:{[n;x] 
   ...
   seq: getinfo[40;n;x;"i";4];
   ack: getinfo[44;n;x;"i";4];
   tsval: getinfo[60;n;x;"i";4];
   tsecr: getinfo[64;n;x;"i";4];
   win: getinfo[50;n;x;"h";2];
...
linuxtokdbtime:{
 // converts time in global header to nanoseconds then accounts for difference in initial times in kdb and linux
...
getcode:{[n;x]
 // code number is stored at 25th byte
 ("i"$1#(globheader+packetheader+25+x[0]) _ n)[0]
 }

getinfo:{[byte;n;x;converttype;bytechunk]
 // mod value is the max value of integers
 ((enlist bytechunk;enlist converttype)1: bytechunk #(globheader+packetheader+byte+x[0]) _ n) mod 4294967296
 };
